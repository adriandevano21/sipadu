...
function createMeeting2($data = array())
{
    $post_time = $data['start_date'];
    $start_time = gmdate("Y-m-d\TH:i:s", strtotime($post_time));
    ...
    $request_url = "https://api.zoom.us/v2/users/" . EMAIL_ID2 . "/meetings";
    $token = array(
        "iss" => API_KEY2,
        "exp" => time() + 3600 //60 seconds as suggested

    );
    $getJWTKey = JWT::encode($token, API_SECRET2);
    $headers = array(
        "authorization: Bearer " . $getJWTKey,
        "content-type: application/json",
        "Accept: application/json",
...
function deleteMeeting($meetingId)
{
    ...
    // $fieldsArr = json_encode($meetingId);
...
function deleteMeeting2($meetingId)
{
