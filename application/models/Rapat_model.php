...
class Rapat_model extends CI_Model
{
  ...
  function get_all()
  {
    return $this->db->select('master_pegawai.nama_pegawai,master_ruang_rapat.*, rapat.*,rapat.topik, master_tim.nama_tim , rapat.id as id_rapat,rapat.lokasi, master_ruang_rapat.id as id_ruangan')
      ->from("rapat")
  ...
  {
    return $this->db->select('topik, tanggal, pukul, selesai, nama_tim, nama_ruangan,urutan, username_notulis, tanggal_selesai,nama_pegawai, username_created,  rapat.id as id_rapat,rapat.lokasi, master_ruang_rapat.id as id_ruangan, count(id_rapat) as peserta')
      ->from("rapat")
      ->join('master_ruang_rapat', 'rapat.id_ruang_rapat=master_ruang_rapat.id', 'left')
      ->join('master_pegawai', 'rapat.username_notulis=master_pegawai.username', 'left')
      ->join('master_tim', 'rapat.id_tim=master_tim.id', 'left')
      ->join('peserta_rapat', 'rapat.id = peserta_rapat.id_rapat', 'left')
      ->like('tanggal', $tahun . '-' . $bulan)
      ->group_by('rapat.id')
      ->order_by("rapat.tanggal", "DESC")
      ->get()->result_array();
  }
  function get_all_bulan_event($bulan, $tahun)
  {
    return $this->db->select('topik, tanggal, pukul, selesai, nama_tim, nama_ruangan,urutan, username_notulis, tanggal_selesai,nama_pegawai, username_created,  rapat.id as id_rapat,rapat.lokasi, master_ruang_rapat.id as id_ruangan, count(id_rapat) as peserta')
      ->from("rapat")
      ->join('master_ruang_rapat', 'rapat.id_ruang_rapat=master_ruang_rapat.id', 'left')
      ...
      ->join('master_tim', 'rapat.id_tim=master_tim.id', 'left')
      ->join('peserta_rapat', 'rapat.id = peserta_rapat.id_rapat', 'left')
    //   ->where('master_ruang_rapat.urutan is NOT NULL', NULL, FALSE)
      ->like('tanggal', $tahun . '-' . $bulan)
  ...
  function get_all_ruang_rapat()
  {
    return $this->db->select('*')
      ...
      ->where('status', 1)
      ->order_by("nama_ruangan")
      ->get()->result_array();
  }
  
  function get_all_ruang_rapat_event()
  {
    return $this->db->select('*')
      ->from("master_ruang_rapat")
      ->where('status', 1)
      ->where('urutan is NOT NULL', NULL, FALSE)
      ->order_by("nama_ruangan")
      ->get()->result_array();
  }
  ...
  {
    $all = $this->db->select('master_tim.nama_tim ,rapat.id_tim,rapat.id_jenis_kegiatan,master_jenis_kegiatan.jenis_kegiatan ,rapat.id, rapat.lokasi, 
    p.nama_pegawai as pengundang, n.nama_pegawai as notulis, c.nama_pegawai as created, nama_ruangan, rapat.id_ruang_rapat,
    topik, tanggal,tanggal_selesai, pukul, notulen, selesai,undangan_rapat.nama_file as nama_file_undangan, 
    notulen_rapat.nama_file as nama_file_notulen, berkas_rapat.nama_file as nama_file_berkas')
      ->from("rapat")
      ...
      ->join('master_ruang_rapat', 'rapat.id_ruang_rapat=master_ruang_rapat.id', 'left') 
      ->join('master_jenis_kegiatan', 'master_jenis_kegiatan.id=rapat.id_jenis_kegiatan', 'left')
  ...
  {
    return $this->db->select('peserta_rapat.username, nama_pegawai,notif_wa, status, master_pegawai.no_wa as no_wa, peserta_rapat.id as id')
      ->from("peserta_rapat")
  ...
  public function  peserta_rapat_id_rapat_belum_wa($id_rapat)
  {
    return $this->db->select('peserta_rapat.username, nama_pegawai, status, master_pegawai.no_wa as no_wa, peserta_rapat.id as id')
      ->from("peserta_rapat")
      ->join('master_pegawai', 'master_pegawai.username=peserta_rapat.username')
      ->where('peserta_rapat.id_rapat', $id_rapat)
      ->where('peserta_rapat.notif_wa', 0)
      ->group_by('peserta_rapat.username')
      ->order_by("master_pegawai.nama_pegawai", "ASC")->get()->result_array();
  }
  ...
  public function get_available_ruangan($tanggal, $pukul_start, $pukul_end)
  {
    ...
    WHERE  mr.status = 1 and mr.id NOT IN (
